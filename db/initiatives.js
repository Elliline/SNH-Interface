...
function recordFollowupTrace(trace = {}) {
  try {
    ...
    // The draft-and-rewrite record. Until 2026-09-17 only `generated` was
    // kept, so a wrong attribution in a sent follow-up could not be traced to
    // the call that produced it — the first draft, the declared cites, and
    // whether a rewrite ran were all in the in-memory trace and nowhere else.
    ...
    db.prepare(`
      INSERT INTO followup_traces
        (id, created_at, conversations_reviewed, message_count, reviewed_json,
         ...
         first_draft, first_draft_problems_json, first_draft_cites_json, cites_json, rewritten, refused_json)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      id,
      ...
      trace.firstDraft || null,
      json(trace.firstDraftProblems),
      json(trace.firstDraftCites),
      json(trace.cites),
      trace.rewritten ? 1 : 0,
      json(trace.refused)
    );
...
function listFollowupTraces({ limit = 20 } = {}) {
  try {
    ...
    return rows.map(r => ({
      ...
      firstDraft: r.first_draft ?? null,
      firstDraftProblems: safeParse(r.first_draft_problems_json, null),
      firstDraftCites: safeParse(r.first_draft_cites_json, null),
      cites: safeParse(r.cites_json, null),
      rewritten: !!r.rewritten,
      refused: safeParse(r.refused_json, null)
    }));
  } catch (error) {
