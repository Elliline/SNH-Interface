...
function sanitizeClusterName(raw) {
  ...
  // Title-case each word, but leave short acronyms (AI, GPU, MSP, CR-V) intact.
  let words = out.map(w =>
    w === '&' ? '&'
      : /^[A-Z0-9]{2,4}$/.test(w) ? w
...
async function generateClusterNameLLM(facts, opts = {}) {
  try {
    ...
    const userPrompt =
      ...
      'Examples:\n' +
      '- facts about a Frontier, a CR-V, and a trailer → Vehicles & Trailer\n' +
      ...
      '- facts about training LLMs and where the field is heading → AI Philosophy\n\n' +
...
const CLUSTER_NAME_MAP = [
  ...
  // Family words match loosely like every other key (plurals, stems), EXCEPT the
  // short ones under exactKeys: "mom" would otherwise hit "moment", "son" hits
  // "song", "mother" hits "motherboard". A whole-word "father" does not match
  // inside "stepfather", so the step- forms are listed on their own.
  { keys: ['father', 'partner', 'family', 'wife', 'husband', 'brother', 'sister', 'kid', 'children',
           'daughter', 'stepdad', 'stepfather', 'stepmom', 'stepmother', 'parent',
           'grandmother', 'grandfather', 'grandma', 'grandpa', 'aunt', 'uncle', 'cousin', 'spouse'],
    exactKeys: ['mom', 'mum', 'dad', 'son', 'mother'],
    name: 'People & Family', plusPersonNames: true },
...
function generateClusterNameFromMembers(members) {
  ...
  const exact = wordMatcher(allText);
  for (const entry of CLUSTER_NAME_MAP) {
    ...
    // Names and the short family words match as whole words only: a name must
    // never be found inside a longer word, and "mom" is not "moment".
    for (const word of exactKeysFor(entry)) if (exact.has(word)) hits++;
...
// Known household names. They come from config rather than living here,
// because they are the user's household, not the program's. Two lists with
// different reach (see the config defaults): personNames feed the
// "People & Family" label above AND isPersonFact below; animalNames feed only
// isPersonFact, so an animal cluster is never labelled as people.
// Tests set this instead of writing the real data/config.json (which SNH_DATA_DIR
// does not redirect); production never touches it.
let householdOverride = null;
function configNames(key) {
  const names = householdOverride ? householdOverride[key] : getConfig().memory?.[key];
  return Array.isArray(names) ? names.map(n => String(n).toLowerCase().trim()).filter(Boolean) : [];
}
function personNames() { return configNames('personNames'); }
function animalNames() { return configNames('animalNames'); }
/** @param {{personNames?: string[], animalNames?: string[]}|null} names - null restores the config. */
function _overrideHouseholdNamesForTests(names) { householdOverride = names; }

/** The words an entry matches as whole words only: its exactKeys, plus the household names where it asks for them. */
function exactKeysFor(entry) {
  return (entry.exactKeys || []).concat(entry.plusPersonNames ? personNames() : []);
}

/**
 * WHOLE-WORD MATCHING for names and the short family words.
 *
 * A name is a word, not a string: it used to be found by substring, so one
 * name matched inside "america" and another inside "lunar", and a two-letter
 * name could not be added at all. This tokenises the text into lowercase
 * words (letters, digits, hyphens), drops a trailing possessive so "Zo's"
 * counts as "zo", and answers whether a name appears as a whole word — or,
 * for a multi-word name, as that exact phrase of consecutive words, or as
 * the words run together ("moss boot" also matches "mossboot").
 */
function wordMatcher(text) {
  ...
  const has = (name) => {
    ...
    if (words.length === 1) return set.has(words[0]);
    if (set.has(words.join(''))) return true;
...
const FAMILY_WORDS = /\b(father|mother|mom|mum|dad|stepdad|stepfather|stepmom|stepmother|parents?|grandmother|grandfather|grandma|grandpa|aunt|uncle|cousin|spouse|partner|wife|husband|son|daughter|brother|sister|kids?|children|cares?\s+for)\b/i;

/**
 * Check if a fact is about a person (contains known names or family terms).
 * Names match as whole words (see wordMatcher); animal names count here so a
 * fact about the household's animals merges with the household's clusters.
 * @param {string} text - Fact text
 * @returns {boolean}
 */
function isPersonFact(text) {
  const exact = wordMatcher(text);
  for (const name of personNames().concat(animalNames())) {
    if (exact.has(name)) return true;
  }
  return FAMILY_WORDS.test(String(text || ''));
}
...
module.exports = {
  ...
  isPersonFact,
  wordMatcher,
  _overrideHouseholdNamesForTests,
