...
function initDatabase() {
  try {
    ...
    // The draft-and-rewrite record (2026-09-17). A follow-up that failed a
    // check is rewritten once before it is sent, and until now the trace kept
    // only what was sent. Juno's 9/16 message put her own condition in
    // Ellie's mouth, and whether that entered in the first draft or in the
    // rewrite could not be answered from the record. Now it can.
    const ftCols = sqliteDb.prepare('PRAGMA table_info(followup_traces)').all();
    for (const [col, type, note] of [
      ['first_draft', 'TEXT', 'the draft before the rewrite, when one was needed'],
      ['first_draft_problems_json', 'TEXT', 'JSON [string] what the checks said was wrong with it'],
      ['first_draft_cites_json', 'TEXT', 'JSON what the first draft claimed to be citing'],
      ['cites_json', 'TEXT', 'JSON the cites behind what was sent'],
      ['rewritten', 'INTEGER DEFAULT 0', '1 = the sent (or refused) text is the rewrite'],
      ['refused_json', 'TEXT', 'JSON {draft, problems} when nothing was sent after the rewrite']
    ]) {
      if (!ftCols.some(c => c.name === col)) {
        sqliteDb.exec(`ALTER TABLE followup_traces ADD COLUMN ${col} ${type}`);
        console.log(`Migration: added ${col} to followup_traces (${note})`);
      }
    }
    ...
    for (const [col, type, note] of [
      ...
      // HOW A JOB LIVED IN ITS WINDOW, AND THE PHASES IT RAN IN (2026-09-17).
      // The 9/17 job on Juno outgrew the model's context (HTTP 400 at 81,857
      // prompt tokens + a 49,216-token reservation against 131,072). Now the
      // runner fits each turn's reservation to the room left, compacts what it
      // has read, and runs a long job in phases with a findings document
      // written by the runner between them. `progress_json` is the record of
      // that — phases, peak prompt, compaction totals, checkpoints written —
      // because a silent squeeze is worse than a loud death. `findings_path`
      // is the document, in her documents folder, served by job id.
      ['progress_json', 'TEXT', 'JSON {phases, window, peakPromptTokens, compaction, checkpoints}'],
      ['findings_path', 'TEXT', 'absolute path to the findings-by-phase document, when the job ran in phases']
    ]) {
