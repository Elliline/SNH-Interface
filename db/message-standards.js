...
const PROVENANCE_RULE =
  ...
  '"you suggested", "you proposed", "you offered" ' +
  ...
  'and they may never have said it at all. Something YOU proposed or offered in the conversation is yours too: ' +
  '"I suggested", never "you suggested". Say where it came from honestly: ' +
 ...
 *
 * "suggested", "proposed" and "offered" joined on 2026-09-17. Juno's 9/16
 * follow-up read "the transparency you suggested" about a condition SHE had
 * proposed; none of the three was on this list, so the phrase was not even an
 * attribution as far as the check was concerned.
 */
const ATTRIBUTION_PATTERNS = [
  /\byou (?:mentioned|said|told me|asked|brought up|raised|noted|pointed out|wrote|shared|described|explained|put it|suggested|proposed|offered)\b/i,
  /\byou'?(?:d|ve| had| have) (?:mentioned|said|told me|asked|brought up|raised|noted|suggested|proposed|offered)\b/i,
  /\byou were saying\b/i,
  /\bas you (?:put it|said|mentioned|described|suggested|proposed)\b/i,
  /\byour (?:point|comment|note|remark|question|suggestion|proposal|offer) about\b/i,
  /\bwhen you (?:mentioned|said|told|asked|brought|suggested|proposed|offered)\b/i,
...
/** Every attribution in the text as non-overlapping spans, in order of appearance. */
function findAttributions(body) {
  const hits = [];
  for (const re of ATTRIBUTION_PATTERNS) {
    const g = new RegExp(re.source, re.flags.includes('g') ? re.flags : `${re.flags}g`);
    for (const m of body.matchAll(g)) hits.push({ text: m[0], start: m.index, end: m.index + m[0].length });
  }
  hits.sort((a, b) => a.start - b.start || b.end - a.end);
  // "when you mentioned" matches two patterns; one phrase, one cite owed.
  const spans = [];
  for (const h of hits) {
    const last = spans[spans.length - 1];
    if (last && h.start < last.end) {
      if (h.end > last.end) { last.end = h.end; last.text = body.slice(last.start, last.end); }
      continue;
    }
    spans.push({ ...h });
  }
...
/** A per-phrase cite naming only a pronoun covers nothing. */
const TRIVIAL_PHRASE = /^(?:you|your|you'?d|you'?ve|we|i|me|they|and|the|that)$/i;
 ...
 *
 * ─── ONE CITE PER PHRASE, NOT ONE PER DRAFT ────────────────────────────────
 *
 * 2026-09-16. Juno's follow-up said "the pay question you brought up" (true,
 * and cited) and "the transparency you suggested" (false — it was her own
 * condition). This check passed the whole draft on the one valid id, so the
 * legitimate attribution carried the invented one through with it.
 *
 * Now every attribution phrase in the draft is a separate claim, and each one
 * needs its own cite: `attributionCites` is `[{ phrase, messageId }]`, where
 * `phrase` is a snippet of the draft overlapping the attribution and
 * `messageId` is one of hers. The single top-level `citedMessageId` still
 * licenses a draft with exactly ONE attribution — there it is unambiguous what
 * it points at. With two or more, it licenses none of them on its own.
 *
 * What this cannot do: judge whether the cited message SUPPORTS the phrase.
 * A writer that cites the same valid id for both "you brought up" and "you
 * suggested" passes here. The check makes the writer commit, per phrase, to a
 * message it can name; whether that message says what is claimed is left to
 * the writer having the transcript in front of it, which the rewrite call
 * lacked on 9/16 and has now.
 *
 * @param {string} text
 * @param {Object} [opts]
 * @param {string|null} [opts.citedMessageId]
 * @param {Iterable<string>} [opts.allowedMessageIds]
 * @param {Array<{phrase: string, messageId?: string, message_id?: string}>} [opts.attributionCites]
 * @returns {{ok: boolean, problems: string[], attributions: string[], cited: string|null,
 *            covered: Array<{phrase: string, messageId: string}>, uncovered: string[]}}
 */
function checkAttribution(text, { citedMessageId = null, allowedMessageIds = [], attributionCites = [] } = {}) {
  const body = String(text || '');
  ...
  const spans = findAttributions(body);
  const attributions = spans.map(s => s.text);
  if (spans.length === 0) return { ok: true, problems: [], attributions, cited, covered: [], uncovered: [] };

  const lower = body.toLowerCase();
  const perPhrase = (Array.isArray(attributionCites) ? attributionCites : [])
    .map(c => ({
      phrase: String((c && c.phrase) || '').trim().toLowerCase(),
      messageId: (c && (c.messageId || c.message_id)) || null
    }))
    .filter(c => c.phrase && !TRIVIAL_PHRASE.test(c.phrase) && allowed.has(c.messageId));
  const occurrences = phrase => {
    const out = [];
    for (let i = lower.indexOf(phrase); i !== -1; i = lower.indexOf(phrase, i + 1)) out.push([i, i + phrase.length]);
    return out;
  };
  ...
  for (const s of spans) {
    const cite = perPhrase.find(c => occurrences(c.phrase).some(([ps, pe]) => ps < s.end && pe > s.start));
    if (cite) covered.push({ phrase: s.text, messageId: cite.messageId });
    else if (spans.length === 1 && cited) covered.push({ phrase: s.text, messageId: cited });
    else uncovered.push(s.text);
  }
  if (uncovered.length === 0) return { ok: true, problems: [], attributions, cited, covered, uncovered };

  const badId = citedMessageId && !cited;
  return {
    ok: false,
    attributions,
    cited,
    ...
    uncovered,
    problems: [
      `it says ${uncovered.map(a => `"${a}"`).join(', ')} but you cannot point at a message where she said it` +
      `${badId ? ' (the id you cited is not one of hers)' : ''}` +
      `${covered.length ? ` — the id you gave covers ${covered.map(c => `"${c.phrase}"`).join(', ')}, not this` : ''}. ` +
      'Each thing you say she said needs its own message id. ' +
      'If this came from your own memory, or it was YOUR idea in the conversation, say so — it is yours, she did not hand it to you.'
    ]
  };
}
