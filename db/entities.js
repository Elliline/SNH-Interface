 ...
 * 2026-09-01: an email from Example Animal Clinic produced an active
 * salience-7 fact asserting the clinic's street address as Inn at Example Cove's
...
function initSchema(db) {
  ...
  // column. "Uses" has no single value on either side: Inn at Example Cove runs
...
function resolve(mention, { type = null } = {}) {
  ...
  // Containment: "Inn at Example Cove" vs a stored "IEC (Inn At Example Cove)".
 ...
 *
 * DIRECTION IS PART OF THE MEANING and it is not symmetric: IEC uses Opera,
 * Opera does not use IEC. from = the user, to = the thing used.
 */
 ...
 * mention resolve.
 *
 * Aliases matter from the first write, not later: "the Inn", "IEC" and "Inn at
 * Example Cove" have to be one entity or the registry grows a duplicate every
...
function createFromMention(mention, { type, orgId = null, relationship = null } = {}) {
  ...
  // "IEC (Inn At Example Cove)" — the longer form is the name, the short one an
 ...
 * fact that merely mentions one is not about it — "User works at IEC" is about
 * Ellie, and handing it to IEC would be the same misattribution the whole
