 ...
 *
 *  - EVENT vs STATE. "User has a pet named Pepper who had a restless night as of
...
Never join unrelated assertions with "and". A clause that describes the SAME one thing stays together: "User has a dog named Biscuit" is one fact, not two.
...
Example: "Its Jaime, i was spelling it wrong lol. She is the Director of Rooms here at IEC." →
  {"fact": "User's colleague Jaime is the Director of Rooms at IEC", "corrects": "earlier facts calling her 'Jamie' or describing her as a manager at IEC"}
...
Respond with exactly YES, NO, or UNCERTAIN on the first line, then one short line of reasoning.`;
    ...
    // strings ("It was Jaime not Jamie" extracts to a fact about Jaime
    // that no longer mentions Jamie) — so pass the source alongside.
    const contextParts = [];
    if (context.corrects) {
...
Answer DIFFERENT when either sentence carries anything the other does not: an extra detail, a narrower or wider claim, a different object, a different attribute. "User has a dog named Biscuit" and "User has a dog named Biscuit who helps pull them up hills" are DIFFERENT — the second knows something the first does not.
 ...
 * dog named Biscuit who helps pull them up hills" arriving against a stored "User
 * has a dog named Biscuit" carries new information and must be stored, so the
...
"User has a dog named Biscuit" and "User has a dog named Biscuit who helps pull them up hills during walks" → the second contains the first entirely, so the answer is whichever letter that longer sentence is.
"User has a dog named Biscuit" and "User has a cat named Mia" → NEITHER.
"User works at IEC" and "User works 20 hours a week" → NEITHER; each carries something the other does not.
...
Answer USER when the sentence describes the human's own life, work, possessions, relationships, body, habits, opinions or preferences about the world. "User has brown eyes", "User has a pet named Pepper", "User prefers to be told directly when she has made a mistake", "User works at IEC", "User is building SNH" — all USER.
 ...
 * "User's grandfather passed away on March 3rd, 2019, at 6:00 AM" — a date-bearing
...
- LASTING — something that stays true. "Her grandfather died on 3 March 2019" → strip the date → "her grandfather died" → still true forever. LASTING. Life events, deaths, births, when something was founded, a diagnosis, a permanent change, a standing habit, a capability someone has — all LASTING, even when the sentence names a date.
...
- A clause describing the SAME single thing stays with it: "${isSelf ? 'I have a habit of pausing when I am unsure, which slows me down' : 'User has a dog named Biscuit who helps pull them up hills'}" is ONE fact.
...
Return ONLY a JSON array of strings.`;
    ...
    // CR-V that has been washed, but the wheels, tires, wax and ceramic coating
    // have not been done" into, among others, "User's CR-V wheels have not been
 ...
 * pending for days before being asked ("What is IEC?" sat 8 days despite a
