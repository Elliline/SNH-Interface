...
function cfg() {
  ...
  return {
    ...
    // THE WINDOW, AND HOW A JOB LIVES INSIDE IT (2026-09-17) — see db/config.js.
    context: {
      windowTokens: Number.isFinite(ctx.windowTokens) && ctx.windowTokens > 0 ? ctx.windowTokens : null,
      marginTokens: Math.max(0, Number.isFinite(ctx.marginTokens) ? ctx.marginTokens : 512),
      floorAnswerTokens: Math.max(64, Number.isFinite(ctx.floorAnswerTokens) ? ctx.floorAnswerTokens : 1024),
      floorThinkingTokens: Math.max(0, Number.isFinite(ctx.floorThinkingTokens) ? ctx.floorThinkingTokens : 1024),
      charsPerToken: Number.isFinite(ctx.charsPerToken) && ctx.charsPerToken > 0.5 ? ctx.charsPerToken : 3
    },
    compaction: {
      enabled: comp.enabled !== false,
      recentRounds: Math.max(0, Number.isFinite(comp.recentRounds) ? comp.recentRounds : 2),
      hardAfter: Math.max(1, Number.isFinite(comp.hardAfter) ? comp.hardAfter : 6),
      snippetChars: Math.max(40, Number.isFinite(comp.snippetChars) ? comp.snippetChars : 160),
      truncateChars: Math.max(200, Number.isFinite(comp.truncateChars) ? comp.truncateChars : 800),
      digestFetches: comp.digestFetches !== false
    },
    phases: {
      enabled: ph.enabled !== false,
      maxPhases: Math.max(1, Number.isFinite(ph.maxPhases) ? ph.maxPhases : 6),
      carryChars: Math.max(1000, Number.isFinite(ph.carryChars) ? ph.carryChars : 12000),
      splitAtPercent: Math.max(0, Math.min(100, Number.isFinite(ph.splitAtPercent) ? ph.splitAtPercent : 70)),
      partsPerPhase: Math.max(1, Number.isFinite(ph.partsPerPhase) ? ph.partsPerPhase : 4)
    }
  };
}
...
function finish(id, { status, resultText = null, error = null, toolCalls = 0, budget = null, stopSource = null, stopKind = null, progress = null, findingsPath = null }) {
  const db = getSqliteDb();
  if (!db) return null;
  const job = getJob(id);
  if (!job) return null;
  ...
  db.prepare(`
    ...
    SET status = ?, finished_at = ?, duration_ms = ?, result_text = ?, error = ?,
        ...
        progress_json = COALESCE(?, progress_json), findings_path = COALESCE(?, findings_path)
    WHERE id = ?
  `).run(status, finishedAt.toISOString(), durationMs, resultText, error,
    toolCalls ?? 0, budget ? JSON.stringify(budget) : null, src, status === 'ok' ? null : stopKind,
    progress ? JSON.stringify(progress) : null, findingsPath || null, id);
...
/**
 * THE PHASE FRAMING. A job that runs in more than one phase is told which one
 * this is, what the earlier ones found (from the document, never the
 * transcript), and that this turn ends with THIS phase's findings — not the
 * report. A one-phase job gets no framing at all and reads exactly as before.
 */
function phaseBlock(phase, phases = [], carry = '', { callsLeft = null } = {}) {
  if (!phase || !phases || phases.length <= 1) return '';
  const later = phases.filter(p => p.n > phase.n).map(p => `${p.n}. ${p.goal}`);
  // A FAIR SHARE, SAID OUT LOUD. The first live run spent 35 of its calls
  // and the whole clock in phase 1 of 6 — a phase told only its goal treats
  // the job's budget as its own. This is guidance, not a cap: the loop
  // enforces nothing here, and a phase that needs more may take more.
  const remaining = phases.filter(p => p.n >= phase.n && p.status !== 'done').length || 1;
  const share = Number.isFinite(callsLeft) && callsLeft > 0 ? Math.max(2, Math.floor(callsLeft / remaining)) : null;
  return (
    `THIS JOB RUNS IN PHASES, one sitting each, and this is PHASE ${phase.n} OF ${phases.length}: "${phase.goal}"` +
    `${phase.parts > 0 ? ` (continued — part ${phase.parts + 1}; what this phase already found is in the document below)` : ''}.\n` +
    `Work ONLY on this phase. When you have what it needs, write THIS PHASE'S FINDINGS as your answer — every ` +
    `claim with its source (title, who, date, URL), what you could not find, and nothing about the phases still to ` +
    `come${later.length ? ` (${later.join('; ')})` : ''}. The final report is put together after the last phase, from the ` +
    `document — do not write it now.\n` +
    (share
      ? `PACE YOURSELF: ${callsLeft} tool call(s) remain for the whole job and ${remaining} phase(s) including this one, ` +
        `so this phase has about ${share} of them. Spend them on what THIS phase asks, write it up, and let the later ` +
        `phases have theirs — a phase that uses the whole budget leaves the rest of the job unanswered.\n`
      : '') +
    (carry
      ? `\nFINDINGS SO FAR, from your own document (written at the end of each earlier phase — build on it, do not redo it):\n${carry}\n\n`
      : '\n')
  );
}

function systemPrompt(job, tools, { previous = null, phase = null, phases = [], carry = '', callsLeft = null } = {}) {
  const now = new Date();
  return (
    ...
    `\n` +
    phaseBlock(phase, phases, carry, { callsLeft }) +
 ...
 */
/** Sum two compaction summaries (db/job-compaction.reportSummary shape). */
function addSummaries(a, b) {
  if (!b) return a || null;
  if (!a) return { ...b };
  const out = { ...a };
  for (const k of Object.keys(b)) if (Number.isFinite(b[k])) out[k] = (Number.isFinite(a[k]) ? a[k] : 0) + b[k];
  return out;
}

/** Sources merged by URL, first appearance kept. */
function mergeSources(a = [], b = []) {
  const seen = new Map();
  for (const s of [...a, ...b]) if (s && s.url && !seen.has(s.url)) seen.set(s.url, s);
  return [...seen.values()];
}

/** The user turn for a phase: the brief, then which phase this sitting is. */
function phaseUserPrompt(job, phase, phases) {
  if (!phase || !phases || phases.length <= 1) return job.task;
  return `${job.task}\n\nTHIS SITTING IS PHASE ${phase.n} OF ${phases.length}: ${phase.goal}\nWork only on this phase, and end by writing this phase's findings with their sources.`;
}

/**
 * ONE MORE TURN, NO TOOLS: write up what this phase has so far. Used when the
 * transcript is past the split point — the phase continues in a fresh sitting
 * with these findings in the document — and the window fit applies to it
 * exactly as to a round, since the transcript is by definition large.
 */
async function phasePartWriteup(convo, phase, c, windowOpt, { calibration = null } = {}) {
  const mm = memoryManager();
  const msgs = [...convo];
  const last = msgs[msgs.length - 1];
  if (last && last.role === 'assistant' && Array.isArray(last.tool_calls) && last.tool_calls.length) {
    for (const call of last.tool_calls) {
      msgs.push({ role: 'tool', tool_call_id: call.id, name: call.function && call.function.name,
        content: JSON.stringify({ error: 'Not run — this sitting is being written up first.' }) });
    }
  }
  msgs.push({
    role: 'user',
    content:
      `STOP looking things up for now — this sitting's transcript has grown large, and the job continues in a fresh one. ` +
      `You have no tools for this turn. Write THIS PHASE'S FINDINGS SO FAR (phase ${phase.n}: ${phase.goal}) from the tool ` +
      `results above: every specific number, date, version, claim and who reported it, each with its source (title, URL, date); ` +
      `then, plainly, what this phase still has to look up. Do not write the final report and do not invent anything. ` +
      `The next sitting starts with these findings in front of it and picks up where you say it should.`
  });
  const res = await mm.callLLM(null, null, {
    continueMessages: msgs, maxTokens: c.answerTokens, thinkingTokens: c.thinkingTokens,
    window: windowOpt ? { ...windowOpt, calibration } : null
  });
  return String(res && res.content || '').trim();
}

/**
 * THE FINAL REPORT, FROM THE DOCUMENT. No tools, no transcript — the phases'
 * own writeups, with their sources, are the whole input, capped to fit the
 * window with the full answer budget kept.
 */
async function synthesise(job, phases, c, windowOpt, allowed) {
  const mm = memoryManager();
  const findings = require('./job-findings');
  const doc = findings.carryBlock(phases, { carryChars: windowOpt && windowOpt.window
    ? Math.max(12000, Math.floor(windowOpt.window * 0.55 * (c.context.charsPerToken || 3)))
    : 60000 });
  const sources = mergeSources(...phases.map(p => p.sources || []));
  const sourceLines = sources.slice(0, 120).map(s => `- ${[s.title, s.url, s.date ? String(s.date).slice(0, 10) : null].filter(Boolean).join(' — ')}`).join('\n');
  const system =
    systemPrompt(job, [], {}) +
    `\n\nHOW THIS RUN WENT: you did it in ${phases.length} phases, one sitting each, and wrote each phase up as you finished ` +
    `it. Those writeups are below, and they are your ONLY material now — you have no tools in this turn. Write the final ` +
    `result from them: keep every specific figure with its source, keep the "could not find" items, and do not add ` +
    `anything the phases did not establish. Where the brief asked for a particular shape (numbered sections, a list at ` +
    `the end), give it that shape.`;
  const user = `FINDINGS BY PHASE (your own writeups):\n\n${doc}\n\n` +
    (sourceLines ? `EVERY SOURCE THE PHASES READ OR FOUND (from the runner's record):\n${sourceLines}\n\n` : '') +
    `Write the final result now.`;
  const res = await mm.callLLM(system, user, { maxTokens: c.answerTokens, thinkingTokens: c.thinkingTokens, window: windowOpt });
  return { text: String(res && res.content || '').trim(), truncated: !!(res && res.truncated) };
}
...
async function runChatHandoff(id, job) {
  ...
  const compaction = require('./job-compaction');
  const findings = require('./job-findings');
  ...
  // --- The window this job runs in -----------------------------------------
  //
  // Known up front (pinned, or read from the engine), so every round's output
  // reservation is fitted to the room left rather than the full budget, and
  // the transcript is compacted before it is sent again. See db/job-window.js
  // and db/job-compaction.js. Neither caps what the job may do: a window that
  // still cannot fit one more turn after compaction fails LOUDLY, with the
  // numbers in the sentence (stop_kind `context-window`).
  const win = await jobWindow.engineWindow();
  const windowOpt = win.window ? {
    window: win.window,
    fit: {
      answerTokens: c.answerTokens, thinkingTokens: c.thinkingTokens,
      floorAnswer: c.context.floorAnswerTokens, floorThinking: c.context.floorThinkingTokens,
      margin: c.context.marginTokens, charsPerToken: c.context.charsPerToken
    }
  } : null;
  // The digest is the job's own work, inside the lane the job already holds —
  // NOT scheduled on the pool. A job awaiting a pool task from inside a pool
  // task deadlocks the moment the pool is throttled to one slot for a chat
  // turn: the job holds the slot, the digest waits for it, the job waits for
  // the digest.
  const digestCall = c.compaction.digestFetches
    ? (sys, user, o) => mm.callLLM(sys, user, { ...o, thinkingTokens: 0 })
    : null;
  const compactor = c.compaction.enabled
    ? (convo, ctx) => compaction.compact(convo, {
      round: ctx.round, hard: ctx.hard, recentRounds: c.compaction.recentRounds, hardAfter: c.compaction.hardAfter,
      snippetChars: c.compaction.snippetChars, truncateChars: c.compaction.truncateChars,
      task: job.task, callLLM: digestCall
    })
    : null;
  ...
  const ask = parseAsk(job.ask_json);
  const ck = mode ? readCheckpoint(id) : null;
  if (mode && !ck) console.warn(`[AgentJobs] ${id.slice(0, 8)} was to resume (${mode}) but has no checkpoint — running from the start`);
  ...
  // --- The phases ------------------------------------------------------------
  //
  // From the checkpoint when this is a resume; from the previous attempt's
  // checkpoint when this is a retry and that attempt finished any phase (the
  // finished ones are carried, the rest start over — "continue from its last
  // checkpoint rather than starting over"); otherwise planned now. A plan of
  // one phase is the job exactly as it ran before phases existed.
  let phases = null;
  let findingsFile = null;
  let plan = null;
  if (ck && Array.isArray(ck.phases) && ck.phases.length) {
    phases = ck.phases;
    findingsFile = ck.findingsPath || null;
    plan = ck.plan || 'resumed';
  } else if (prevCk && Array.isArray(prevCk.phases) && prevCk.phases.some(p => p.status === 'done')) {
    phases = prevCk.phases.map(p => p.status === 'done' ? { ...p } : findings.newPhase(p.n, p.goal));
    plan = `carried ${phases.filter(p => p.status === 'done').length} finished phase(s) from the previous attempt`;
    console.log(`[AgentJobs] ${id.slice(0, 8)} retry: ${plan}`);
  } else if (c.phases.enabled) {
    const planned = await findings.planPhases(job, {
      callLLM: (sys, user, o) => mm.callLLM(sys, user, { ...o, thinkingTokens: c.thinkingTokens ? Math.min(c.thinkingTokens, 2048) : 0 }),
      maxPhases: c.phases.maxPhases
    });
    phases = planned.phases;
    plan = planned.why;
    console.log(`[AgentJobs] ${id.slice(0, 8)} phase plan: ${plan}${phases.length > 1 ? ` — ${phases.map(p => `${p.n}. ${p.goal}`).join('; ')}` : ''}`);
  } else {
    phases = [findings.newPhase(1, 'the whole job')];
    plan = 'phases off';
  }
  const multi = phases.length > 1;
  if (multi && !findingsFile) findingsFile = findings.findingsPath(job);

  // What the row and the card get: the phases, the window, the compaction
  // totals, how many checkpoints were written — visible, per the brief.
  const totals = { peakPromptTokens: 0, peakCompletionTokens: 0, squeezedRounds: 0, refitRetries: 0, compaction: null, checkpoints: 0, attemptCalls: 0 };
  const noteRun = (res) => {
    const st = res && res.stats;
    if (st) {
      totals.peakPromptTokens = Math.max(totals.peakPromptTokens, st.peakPromptTokens | 0);
      totals.peakCompletionTokens = Math.max(totals.peakCompletionTokens, st.peakCompletionTokens | 0);
      totals.squeezedRounds += st.squeezedRounds | 0;
      totals.refitRetries += st.refitRetries | 0;
    }
    if (res && res.compaction) totals.compaction = addSummaries(totals.compaction, res.compaction);
  };
  const progress = () => ({
    phases: findings.phaseSummary(phases), plan, multi,
    window: { tokens: win.window, source: win.source }, peakPromptTokens: totals.peakPromptTokens,
    peakCompletionTokens: totals.peakCompletionTokens, reservation: c.answerTokens + (c.thinkingTokens || 0),
    squeezedRounds: totals.squeezedRounds, refitRetries: totals.refitRetries,
    compaction: totals.compaction, checkpoints: totals.checkpoints
  });
  const writeDoc = (stop = null) => {
    if (!multi) return;
    if (findings.writeDocument(findingsFile, findings.renderDocument({ job, phases, stop }))) totals.checkpoints++;
  };
  const checkpointFor = (phase) => (state) => writeCheckpoint(id, { ...state, title: job.title, task: job.task, phases, phase: phase.n, findingsPath: findingsFile, plan, totals });
  if (ck && ck.totals) Object.assign(totals, ck.totals, { compaction: ck.totals.compaction || null });

  // --- She said NO: one writeup turn, no tools, and it is over ---------------
  if (mode === 'declined' && ck && Array.isArray(ck.convo) && ck.convo.length) {
    session.restore(ck.session);
    return wrapUpDeclined(id, job, ck, session, c, { phases, findingsFile, windowOpt, progress, writeDoc });
  }

  const askPct = c.askBeforeCeiling ? c.askAtPercent : 0;
  const pauseWhen = (sess) => sess.nearing(askPct);
  const splitWhen = multi && windowOpt && c.phases.splitAtPercent > 0 && c.phases.splitAtPercent < 100
    ? ({ promptTokens, window, round }) => promptTokens > window * (c.phases.splitAtPercent / 100)
      ? `the transcript reached ~${promptTokens.toLocaleString('en-US')} of ${window.toLocaleString('en-US')} tokens before round ${round} (the split point is ${c.phases.splitAtPercent}%)`
      : null
    : null;

  console.log(`[AgentJobs] === running ${id.slice(0, 8)}: "${job.title}"${mode ? ` (${mode})` : ''}${multi ? ` in ${phases.length} phases` : ''}${win.window ? `, window ${win.window} (${win.source})` : ''} ===`);

  let status = 'ok', error = null, output = '', budget = null, toolCalls = 0;
  let stopSource = null, stopKind = null;
  let calls = [];            // the CURRENT sitting's tool record — what a salvage writes from
  let lastRes = null;
  let current = null;
  const phaseStops = [];
  try {
    for (const phase of phases) {
      ...
      // A resume applies to the sitting the checkpoint was written in.
      let resume = null;
      if (mode && ck && Array.isArray(ck.convo) && ck.convo.length && (ck.phase || 1) === phase.n) {
        session.restore(ck.session);
        // Only the calibration is carried into the loop. Its counters and
        // compaction totals start at zero for the resumed sitting: the job's
        // totals (restored from ck.totals above) already hold what came
        // before, and carrying them into the loop as well would count every
        // squeezed round and compacted result twice.
        const carried = { calibration: ck.calibration || null };
        if (mode === 'granted') {
          const g = (ask && ask.grant) || {};
          session.extend({ calls: g.calls || 0, rounds: g.rounds || 0, wallMs: g.wallMs || 0 });
          resume = {
            convo: ck.convo, toolCalls: ck.toolCalls || [], pendingCalls: ck.pendingCalls || [], ...carried,
            note: `Ellie said yes. You have ${g.calls || 0} more tool call(s), ${g.rounds || 0} more round(s) and ` +
              ...
              `${session.billed.toFixed(1)} of ${session.maxCalls} calls used so far. Carry on and finish${multi ? ' this phase' : ' the job'}; ` +
              `write the ${multi ? "phase's findings" : 'result'} when you have it.`
          };
          ...
          resume = {
            convo: t.convo, toolCalls: t.toolCalls, pendingCalls: [], ...carried,
      ...
      // The sittings of one phase. Usually one; a split makes another.
      let res;
      for (;;) {
        const carry = findings.carryBlock(phases, { carryChars: c.phases.carryChars });
        res = await mm.callLLM(
          systemPrompt(job, allowed, { previous, phase, phases, carry, callsLeft: Math.max(0, Math.floor(session.maxCalls - session.billed)) }),
          phaseUserPrompt(job, phase, phases),
          {
            maxTokens: c.answerTokens, thinkingTokens: c.thinkingTokens, toolSession: session,
            resume: resume ? { convo: resume.convo, toolCalls: resume.toolCalls, pendingCalls: resume.pendingCalls, note: resume.note, calibration: resume.calibration } : null,
            checkpoint: checkpointFor(phase), pauseWhen, window: windowOpt, compactor, splitWhen
          }
        );
        resume = null;
        noteRun(res);
        ...
        phase.calls += calls.length;
        // Tool rounds, not requests: the writeup turn is not a round.
        phase.rounds += new Set(calls.map(k => k.round).filter(Number.isFinite)).size;
        phase.sources = mergeSources(phase.sources, compaction.sourcesIn(res && res.convo || []));
        totals.attemptCalls += calls.length;
        ...
        // === PAST THE SPLIT POINT: WRITE THIS SITTING UP, START ANOTHER ======
        if (res && res.split) {
          const part = await phasePartWriteup(res.convo, phase, c, windowOpt, { calibration: res.stats && res.stats.calibration });
          phase.parts = (phase.parts || 0) + 1;
          phase.writeup = [phase.writeup, part ? `_(sitting ${phase.parts})_\n\n${part}` : null].filter(Boolean).join('\n\n');
          writeDoc(null);
          const line = `Background job ${id.slice(0, 8)} phase ${phase.n} split after sitting ${phase.parts}: ${res.splitWhy}. Its findings so far are in ${findingsFile}; the phase continues in a fresh sitting.`;
          console.log(`[AgentJobs] ${line}`);
          opsLog(line);
          if (phase.parts >= c.phases.partsPerPhase) {
            phase.stop = `it was split ${phase.parts} times, the limit (agentJobs.phases.partsPerPhase) — what it had is kept`;
            break;
          }
          continue;
        }
      ...
      if (content) {
        phase.writeup = [phase.writeup, phase.parts ? `_(sitting ${phase.parts + 1})_\n\n${content}` : content].filter(Boolean).join('\n\n');
        phase.status = 'done';
      } else {
        phase.status = 'stopped';
        phase.stop = phase.stop || (cut ? cut.plain : 'it wrote nothing for this phase');
      }
      phase.endedAt = new Date().toISOString();
      writeDoc(null);
      if (multi) console.log(`[AgentJobs] ${id.slice(0, 8)} phase ${phase.n}/${phases.length} ${phase.status}: ${phase.calls} call(s), ${phase.rounds} round(s), ${(phase.sources || []).length} source(s)`);

      // WHAT A CUT MEANS IN A PHASED JOB. The rounds limit and the answer
      // budget are per SITTING — a phase that hit one wrote up what it had and
      // the next phase gets fresh rounds, so the job goes on and the card says
      // which phase was cut. The job-wide budgets (calls, the clock) bind the
      // whole job: nothing more can run, the phases after are not reached, and
      // the card says so with what the finished ones found — as loudly as
      // before, never quieter.
      if (multi && cut && (cut.kind === 'rounds' || cut.kind === 'answer-budget')) {
        phase.stop = `${cut.plain}`;
        phaseStops.push(`phase ${phase.n} — ${cut.plain}`);
      } else if (cut || !content) {
        for (const p of phases) if (p.status === 'pending') p.status = 'skipped';
        if (cut) { status = 'partial'; stopSource = cut.source; stopKind = cut.kind; error = `${cut.plain} What is above is what it had.`; }
        break;
      }
      // Rounds are per sitting; the calls and the clock are the session's,
      // shared across phases, and keep counting.
      session.roundsUsed = 0;
    }
    if (status === 'ok' && phaseStops.length) {
      status = 'partial'; stopSource = 'runner'; stopKind = 'rounds';
      error = `Every phase ran, but ${phaseStops.length} of them ${phaseStops.length === 1 ? 'was' : 'were'} cut short: ${phaseStops.join('; ')}. The rest is complete.`;
    }

    // === WHAT SHE READS ======================================================
    if (!multi) {
      output = phases[0].writeup || '';
    } else if (phases.every(p => p.status === 'done')) {
      try {
        const syn = await synthesise(job, phases, c, windowOpt, allowed);
        output = syn.text;
        if (output && syn.truncated) {
          status = 'partial'; stopSource = 'runner'; stopKind = 'answer-budget';
          error = `The final report reached the answer budget (${c.answerTokens} tokens) and was cut off. The full findings of every phase are in the document.`;
        }
      } catch (err) {
        console.warn(`[AgentJobs] ${id.slice(0, 8)} synthesis failed, assembling the phases instead: ${err.message}`);
      }
      if (!output) output = findings.assembleResult({ phases, stop: null });
    } else {
      output = findings.assembleResult({ phases, stop: error });
      if (status === 'ok') { status = 'partial'; stopSource = stopSource || 'runner'; stopKind = stopKind || 'no-answer'; error = error || 'It stopped without finishing every phase.'; }
    }
    toolCalls = multi ? phases.reduce((n, p) => n + (p.calls || 0), 0) : calls.length;

    if (output && status === 'ok') {
      // A single-phase run that was CUT SHORT but still wrote something is not
      // "ok" — same three ways as before (answer budget, rounds, the budget).
      if (!multi) {
        const cut = require('./job-failure').classifyCutShort({
          truncated: !!(lastRes && lastRes.truncated), outOfRounds: !!(lastRes && lastRes.outOfRounds), budget,
          answerTokens: c.answerTokens, maxRounds: session.maxRounds
        });
        if (cut) { status = 'partial'; stopSource = cut.source; stopKind = cut.kind; error = `${cut.plain} What is above is what it had.`; }
      }
    ...
    if (!output) {
      ...
      // memory work it had already completed went in the bin with it. Two
      // attempts, in order of how much they can say: ask it to write up what it
      // has (one more call, no tools, the run's own transcript), and failing
      // that write the account ourselves — deterministic, cannot come back empty.
      const salvaged = await salvageWriteup(job, calls, budget, c, { convo: lastRes && lastRes.convo });
      const stop = require('./job-failure').classifyBudgetStop(budget, { maxRounds: session.maxRounds }) || { source: 'runner', kind: 'no-answer', plain: 'It stopped without writing an answer.' };
      stopSource = stopSource || stop.source; stopKind = stopKind || stop.kind;
      status = 'partial';
      error = error || `${stop.plain} ${describeStop(calls, null, c)}`;
      if (salvaged) {
        output = multi ? findings.assembleResult({ phases, partial: salvaged, stop: error }) : salvaged;
        console.warn(`[AgentJobs] ${id.slice(0, 8)} produced no answer — salvaged a writeup from ${calls.length} tool call(s)`);
      } else {
        const account = mechanicalAccount(job, calls, budget, c);
        output = multi ? findings.assembleResult({ phases, partial: account, stop: error }) : account;
    ...
    if (err && err.stats) noteRun({ stats: err.stats, compaction: err.compaction });
    if (current) { current.calls = Math.max(current.calls || 0, calls.length); current.status = 'stopped'; current.endedAt = new Date().toISOString(); }
    toolCalls = multi ? phases.reduce((n, p) => n + (p.calls || 0), 0) : calls.length;
    ...
    const failure = require('./job-failure').classifyThrown(err, {
      round: err && err.round, calls: calls.length,
      recentRestart: (() => { try { return require('./brain-watchdog').recentRestart(); } catch { return null; } })(),
      formatTime: (ms) => formatLocalTime(new Date(ms), { style: 'time', fallback: 'an unclear time' })
    });
    stopSource = failure.source; stopKind = failure.kind;
    error = failure.plain;
    if (current) current.stop = current.stop || failure.plain;
    for (const p of phases) if (p.status === 'pending') p.status = 'skipped';
    ...
    // before the cut is kept, labelled as partial — and, in a phased job, the
    // phases it FINISHED are on the card and in the document regardless.
    ...
    const partial = salvaged || mechanicalAccount(job, calls, budget, c, error, { partialText });
    if (current && multi) current.sources = mergeSources(current.sources, compaction.sourcesIn(convo || []));
    output = multi ? findings.assembleResult({ phases, partial, stop: error }) : partial;
    writeDoc(error);
  }

  const done = finish(id, { status, resultText: output || null, error, toolCalls, budget, stopSource, stopKind, progress: progress(), findingsPath: multi ? findingsFile : null });
  // The file comes after the status is settled — see attachArtifact.
  const withFile = done ? await attachArtifact(id, { note: error }) : null;
  const secs = done ? (done.duration_ms / 1000).toFixed(1) : '?';
  const ctxLine = totals.peakPromptTokens ? ` Peak prompt ${totals.peakPromptTokens} of ${win.window || '?'} tokens` +
    `${totals.compaction && totals.compaction.entries ? `, ${totals.compaction.roundsCompacted} round(s) compacted (${totals.compaction.droppedChars} chars dropped)` : ''}` +
    `${multi ? `, ${totals.checkpoints} checkpoint(s) to ${findingsFile}` : ''}.` : '';
  const line = status === 'ok'
    ? `Background job finished: "${job.title}" (${id.slice(0, 8)}) — ok in ${secs}s, ${toolCalls} tool call(s)${multi ? `, ${phases.length} phases` : ''}.${ctxLine}`
    : status === 'partial'
      ? `Background job finished PARTIAL: "${job.title}" (${id.slice(0, 8)}) — ${secs}s, ${toolCalls} tool call(s). It wrote up what it had. Why it stopped: ${error}${ctxLine}`
      : `Background job FAILED: "${job.title}" (${id.slice(0, 8)}) — ${error}${multi ? ` Finished phases are in ${findingsFile}.` : ''}${ctxLine}`;
  console.log(`[AgentJobs] ${line}`);
  opsLog(line);
 ...
 */
async function wrapUpDeclined(id, job, ck, session, c, { phases = null, findingsFile = null, windowOpt = null, progress = null, writeDoc = null } = {}) {
  const mm = memoryManager();
  const { sayDuration } = require('./job-failure');
  const ask = parseAsk(job.ask_json) || {};
  const convo = [...ck.convo];
  const multi = Array.isArray(phases) && phases.length > 1;
  const current = multi ? phases.find(p => p.n === (ck.phase || 1)) : null;
  ...
  try {
    const r = await mm.callLLM(null, null, { continueMessages: convo, maxTokens: c.answerTokens, thinkingTokens: c.thinkingTokens, window: windowOpt ? { ...windowOpt, calibration: ck.calibration || null } : null });
  ...
  if (multi) {
    // The phase it was in gets this writeup; the finished ones are already in
    // the document, and the card carries all of it.
    if (current) {
      current.status = 'stopped'; current.stop = 'Ellie declined more budget in this phase';
      current.calls = Math.max(current.calls || 0, calls.length);
      current.sources = mergeSources(current.sources, require('./job-compaction').sourcesIn(ck.convo || []));
    }
    for (const p of phases) if (p.status === 'pending') p.status = 'skipped';
    if (typeof writeDoc === 'function') writeDoc(error);
    output = require('./job-findings').assembleResult({ phases, partial: output, stop: error });
  }
  const done = finish(id, {
    status: 'partial', resultText: output, error,
    toolCalls: multi ? phases.reduce((n, p) => n + (p.calls || 0), 0) : calls.length,
    budget: session.summary(), stopSource: 'user', stopKind: 'budget-declined',
    progress: typeof progress === 'function' ? progress() : null, findingsPath: multi ? findingsFile : null
  });
  const withFile = done ? await attachArtifact(id, { note: error }) : null;
...
function sweepInterrupted({ now = new Date() } = {}) {
  ...
  for (const j of open) {
    ...
    if (retryable) {
      ...
      const account = hasRecord && calls.length ? mechanicalAccount(j, calls, ck.session ? { ...ck.session, exhausted: null } : null, c, 'SNH restarted') : null;
      // A PHASED JOB'S FINISHED PHASES ARE ON THE CARD, whatever happened to
      // the one it was in. The document on disk already has them; the card
      // must not say less than the document does.
      const phased = !isReview && ck && Array.isArray(ck.phases) && ck.phases.length > 1;
      const partialText = isReview
        ...
        : phased
          ? require('./job-findings').assembleResult({
            phases: ck.phases.map(ph => ph.status === 'running' ? { ...ph, status: 'stopped', stop: 'SNH restarted while it was in this phase' } : ph),
            partial: account, stop: why })
          : account;
      finish(j.id, {
        ...
        progress: phased ? { phases: require('./job-findings').phaseSummary(ck.phases), multi: true, plan: ck.plan || null, ...(ck.totals || {}) } : null,
        findingsPath: phased ? (ck.findingsPath || null) : null
      });
...
function feed({ limit = 50 } = {}) {
  ...
  const jobs = db.prepare(
    ...
    // HOW IT LIVED IN THE WINDOW, and the phases it ran in (2026-09-17). The
    // findings document is reachable by job id, never by path — same rule as
    // the result file.
    progress: (() => { try { return j.progress_json ? JSON.parse(j.progress_json) : null; } catch { return null; } })(),
    findings_name: j.findings_path ? path.basename(j.findings_path) : null,
    findings_location: j.findings_path ? path.dirname(j.findings_path) : null
  }));
  ...
  `).all(...RS, lim).map(r => ({
    ...
    summary_text: null,
    progress: null, findings_name: null, findings_location: null
  }));
