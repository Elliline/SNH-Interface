 ...
 * how the canoe triple and the Biscuit subset finally go.
 *
 ...
 * relation rather than weighed: "has a dog named Biscuit who helps pull them up
 * hills" absorbs "has a dog named Biscuit", never the reverse. Detail is never
...
async function mergeNearDuplicates(pass, { subject = null } = {}) {
  ...
  for (const row of rows) {
    ...
    for (const cand of candidates) {
      ...
      // Biscuit pair as DIFFERENT on purpose, because at INTAKE the richer
...
async function expireDatedEvents(pass, { subject = null } = {}) {
  ...
  for (const row of rows) {
    ...
    // proposed retiring "User's grandfather passed away on March 3rd, 2019" and
...
async function repairCompounds(pass, { subject = null } = {}) {
  ...
  for (let start = 0; start < candidates.length; start += chunk) {
    ...
    // chain that retired "User (Ellie) has brown eyes and her favorite color is
    // green" for "User's favorite color is purple".
    ...
    for (const item of slice) {
      ...
      // be "User's grandfather passed away on March 3rd, 2019". Leaving the
...
async function runPass(opts = {}) {
  ...
  try {
    ...
    // clauses nobody disputed with it — "User (Ellie) has brown eyes and her
    // favorite color is orange" was retired over the colour, and the eye colour
    // went with it.
    //
