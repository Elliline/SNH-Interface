...
const DEFAULTS = {
  ...
  agentJobs: {
    ...
    retentionDays: 90,
    // ============ THE WINDOW, AND HOW A JOB LIVES INSIDE IT (2026-09-17) ============
    //
    // The 9/17 job on Juno died in tool round 11, 40 calls in: HTTP 400,
    // "maximum context length is 131072 tokens. However, you requested 49216
    // output tokens and your prompt contains at least 81857". Not a timeout to
    // raise — 131k is the model's ceiling. The fix is spending it properly and
    // not needing it all at once, in three parts, each in its own block below.
    // NONE OF THEM CAPS WHAT A JOB MAY DO. A window that cannot fit one more
    // turn after all three fails loudly with the numbers (stop_kind
    // `context-window`), the way the 9/17 card did.
    //
    // PART 1 — the reservation fits. Every round used to reserve the full
    // answer+thinking budget (49,216 on Juno) whether or not the turn would
    // produce it; a tool round on that transcript produced ~450 tokens. Now the
    // runner knows the ceiling (windowTokens, or the engine's max_model_len)
    // and gives a round the full budget only while it fits, then only what
    // fits, down to the floors. The estimate is calibrated from the engine's
    // own `usage` count every round — the 9/17 transcript ran 2.4 chars/token,
    // not 4.
    context: {
      // null = ask the engine (db/model-context, the same probe the chat window
      // uses; cached). A number pins it — also how a 262k box rehearses the
      // 131k one. Settings → Agent Jobs shows what the engine answered.
      windowTokens: null,
      // Headroom under the ceiling the estimate cannot see: chat-template
      // tokens, the tool-call framing. 512 is generous for Qwen's template.
      marginTokens: 512,
      // The least a squeezed round may be given. A tool round needs a few
      // hundred tokens for its calls; a writeup that lands squeezed is
      // re-asked after compaction, so this is not the size of any answer.
      floorAnswerTokens: 1024,
      floorThinkingTokens: 1024,
      // The first-round guess at chars per token, before the engine has said.
      // Low on purpose: tool JSON, URLs and code tokenise densely.
      charsPerToken: 3
    },
    // PART 2 — tool results are compacted as the job runs. After a search or a
    // fetch has been read, a later round needs what was LEARNED and WHERE FROM,
    // not the page: a search keeps every hit's title, url, date and a short
    // snippet; a fetched page becomes a digest of its findings (one model call
    // per round of pages, thinking off) with the title/url/date attached by
    // the runner from the raw result, so provenance cannot be lost. Older
    // rounds compact harder. The report (rounds compacted, chars dropped,
    // digested vs truncated) is on the row and the card.
    compaction: {
      enabled: true,
      // Rounds behind the current one that stay raw, so the model can still
      // read what it just fetched. 2 = the page it fetched last round and the
      // round before are untouched.
      recentRounds: 2,
      // Rounds behind the current one after which the harder tier applies:
      // reasoning dropped from those assistant turns, search hits cut to five.
      hardAfter: 6,
      snippetChars: 160,
      // When a page cannot be digested (the digest call failed), it is cut to
      // this many chars of its head instead, and the report counts it.
      truncateChars: 800,
      // Off = never call the model for a digest; every old page is truncated.
      // Cheaper, and it loses the findings below the head of the page.
      digestFetches: true
    },
    // PART 3 — a long job is not one context window. One planning call splits
    // the brief into phases (a one-question brief is ONE phase and runs exactly
    // as before). Each phase is a fresh transcript: the brief, the phase's
    // goal, and the findings document so far. When a phase writes itself up,
    // THE RUNNER appends it to the document — with the sources the phase read,
    // from the tool record — and starts the next phase clean. A job that dies
    // in phase 3 has phases 1 and 2 on disk and on its card. Tool rounds are
    // per sitting; the call budget and the clock are per job.
    phases: {
      enabled: true,
      maxPhases: 6,
      // How much of the document the next phase is shown verbatim (the most
      // recent phases first; older ones cut to their opening past this).
      carryChars: 12000,
      // A sitting past this share of the window is written up and continued
      // fresh — before the wall, not at it. 0 or 100 = never split early.
      splitAtPercent: 70,
      // Sittings one phase may take before it is closed with what it has.
      partsPerPhase: 4
    }
  },
  // ============ Job output as FILES ============
  ...
  memory: {
    ...
    // First names (lowercase) of the household, used only by cluster naming.
    // Two lists because they are used differently, and were before they moved
    // here: personNames count as keys for the "People & Family" label AND mark
    // a fact as being about a person when singleton clusters are merged;
    // animalNames do only the second. An animal's name must not pull a cluster
    // into "People & Family" — three animals and "dog"/"cat" is "Pets & Animals".
    // Names match as whole words (a two-word name matches as the phrase or run
    // together), never inside a longer word, so short names are safe here.
    // Both empty by default because they are personal; set them in
    // data/config.json.
    personNames: [],
    animalNames: [],
