...
(async () => {
  ...
  if (fixtures && fixtures.results && fixtures.results.length) {
    ...
    // and a corpus holding no fact about Biscuit at all satisfies it without
    // testing anything. The checker cannot tell "the defect was repaired" from
    // "the subject never arrived", so this does.
    const biscuit = fixtures.results.find(r => r.id === 'F4');
    const biscuitCount = biscuit && (biscuit.notes || []).find(n => /active facts naming Biscuit/.test(n));
    if (biscuit && biscuit.pass && /:\s*0$/.test(String(biscuitCount || '').trim())) {
      say('');
      say('  F4 passes VACUOUSLY. It asks whether a fact about Biscuit is sitting beside a');
      say('  poorer copy of itself, and staging holds no fact about Biscuit at all — there');
      say('  is nothing for the rule to be true or false about. The reason is below: the');
      say('  Biscuit fact is in ELLIE-DECIDES, unmarked. Carry it and F4 becomes a real');
  ...
  if (!corr) {
    ...
    if (retired.length) {
      ...
      // what is. "User had to get rid of the CR-V and the Ridgeline due to painful
      // memories associated with a move" and "User has a CR-V" are both true;
      ...
      if (historical.length) {
        ...
        say('    has a CR-V" are both true — different vehicles, different times — and');
        ...
        say('    June 2026, User purchased a Nissan Frontier" was about to be repaired onto the');
        say('    carried Pilot fact, and repoint refused because that fact had just been');
  ...
  if (!plan) {
    ...
    if (archived.length) {
      ...
      say('   the brown eyes, the pet named Pepper, wanting to be told directly about a mistake.)');
      say('');
      ...
      say('  these from the genuine ones — "User has a pet named Pepper" and "User has blue');
