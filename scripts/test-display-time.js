...
console.log('\n5. The digest — the surface where this was caught');
{
  ...
  const mid = database.addMessage(conv, 'assistant',
    'Maple was already at the door before the coffee finished, which is the most Beagle thing possible.');
  db.prepare('UPDATE messages SET timestamp = ? WHERE id = ?').run('2026-08-27 13:09:12', mid);

  memoryManager.callLLM = async () => ({
    content: JSON.stringify({
      found: true,
      summary: 'You talked about Maple in the morning.',
      quotes: [{ message_id: mid, quote: 'Maple was already at the door before the coffee finished' }],
      gaps: ''
    }),
  ...
  const hits = historySearch.find({ query: 'Beagle coffee door' });
  ...
  tz = 'America/New_York';
  const east = historySearch.find({ query: 'Beagle coffee door' }).hits.find(h => h.message_id === mid);
