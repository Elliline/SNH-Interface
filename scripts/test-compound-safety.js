 ...
 *
 * "User (Ellie) has brown eyes and her favorite color is orange (last updated
 * 2026-07-27)" was retired for "User's favorite color is purple". The colour was
 ...
 * in docs/staging-gate4-report.txt: the compound split into "User has brown eyes."
 * and "User's favorite color is orange (last updated 2026-07-27)", and only the
...
check('A1 the exact sentence that lost its eye colour',
  strip('User (Ellie) has brown eyes and her favorite color is orange (last updated 2026-07-27).').text,
  'User has brown eyes and her favorite color is orange (last updated 2026-07-27).');

check('A2 …and it reports what it removed',
  strip('User (Ellie) has brown eyes.').stripped, 'Ellie');
...
check('A4 an unannotated fact is untouched',
  strip('User has brown eyes').text, 'User has brown eyes');

check('A5 …and reports nothing removed',
  strip('User has brown eyes').stripped, null);
...
check('A7 a parenthetical that is not right after the subject is not stripped',
  strip('User has a dog (Biscuit) who pulls them up hills').text,
  'User has a dog (Biscuit) who pulls them up hills');

check('A8 a multi-word parenthetical is not a bare name',
  strip('User (the primary account holder) has brown eyes').stripped, null);

// --- 2. the guard that stops a compound losing whole -----------------------
check('C1 the blue-eyes sentence reads as a compound',
  rules.looksCompound('User (Ellie) has brown eyes and her favorite color is orange (last updated 2026-07-27).').compound,
  true);

check('C2 …and still does once the annotation is gone',
  rules.looksCompound(strip('User (Ellie) has brown eyes and her favorite color is orange (last updated 2026-07-27).').text).compound,
  true);

check('C3 the clause on its own is NOT a compound, so it may be superseded',
  rules.looksCompound("User's favorite color is orange (last updated 2026-07-27).").compound,
  false);

check('C4 the other clause is not a compound either',
  rules.looksCompound('User has brown eyes.').compound, false);

check('C5 the fact that won is not a compound',
  rules.looksCompound("User's favorite color is purple").compound, false);
