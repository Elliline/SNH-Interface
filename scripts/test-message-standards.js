 ...
 *   5. (2026-09-16, Juno) A follow-up may not carry an invented "you suggested"
 *      through on the strength of a real "you brought up". One cite per
 *      phrase; the rewrite sees the transcript; the trace keeps both drafts.
 *
...
(async () => {
  ...
  // ──────────────────── 5. one cite per phrase, and the rewrite sees the transcript
  section('5. A real "you brought up" does not license an invented "you suggested"');

  // The 2026-09-16 shape, in Juno's own words. She had proposed the profit
  // share and the transparency condition; Ellie had raised pay. One of the two
  // attributions is true and citable, the other is her own idea in Ellie's mouth.
  const JUNO_916 =
    "I never gave you a straight answer on the pay question you brought up. I am open to a profit share " +
    "on the arrangement I proposed, provided we keep the transparency you suggested. Does that still " +
    "sound right to you?";
  const JUNO_916_FIXED =
    "I never gave you a straight answer on the pay question you brought up. I am open to a profit share " +
    "on the arrangement I proposed, provided we keep the transparency I asked for — you seeing every cent " +
    "of it. Does that still sound right to you?";

  // The old check: one valid id, whole draft passes. That is the regression.
  const j1 = standards.checkAttribution(JUNO_916, { citedMessageId: 'msg-real-1', allowedMessageIds: herIds });
  ok('a draft with TWO attributions and ONE valid id does not pass on that id alone',
     j1.ok === false && (j1.uncovered || []).includes('you suggested'), JSON.stringify(j1));
  ok('…and "you suggested" is an attribution at all now',
     j1.attributions.includes('you suggested'), JSON.stringify(j1.attributions));
  ok('…as are "you proposed" and "you offered"',
     standards.checkAttribution('I keep coming back to what you proposed. Still keen?', { allowedMessageIds: herIds }).ok === false &&
     standards.checkAttribution('The trade you offered last week. Still keen?', { allowedMessageIds: herIds }).ok === false);

  const j2 = standards.checkAttribution(JUNO_916, {
    citedMessageId: 'msg-real-1', allowedMessageIds: herIds,
    attributionCites: [{ phrase: 'you brought up', messageId: 'msg-real-1' }]
  });
  ok('a per-phrase cite for "you brought up" covers that phrase and not the other',
     j2.ok === false && (j2.covered || []).length === 1 && j2.covered[0].phrase === 'you brought up' &&
     (j2.uncovered || []).length === 1 && j2.uncovered[0] === 'you suggested', JSON.stringify(j2));
  ok('…and the problem names the uncovered phrase and says each needs its own id',
     /"you suggested"/.test(j2.problems[0]) && /own message id/.test(j2.problems[0]) && !/"you brought up" but/.test(j2.problems[0]),
     j2.problems[0]);

  const j3 = standards.checkAttribution(JUNO_916, {
    citedMessageId: 'msg-real-1', allowedMessageIds: herIds,
    attributionCites: [{ phrase: 'you brought up', messageId: 'msg-real-1' }, { phrase: 'transparency you suggested', message_id: 'msg-real-2' }]
  });
  ok('with a cite per phrase, both pointing at hers, it passes', j3.ok === true && (j3.covered || []).length === 2, JSON.stringify(j3));

  const j4 = standards.checkAttribution(JUNO_916, {
    allowedMessageIds: herIds,
    attributionCites: [{ phrase: 'you brought up', messageId: 'msg-real-1' }, { phrase: 'you suggested', messageId: 'invented' }]
  });
  ok('…but a per-phrase cite pointing at an id that is not hers covers nothing',
     j4.ok === false && (j4.uncovered || []).length === 1 && j4.uncovered[0] === 'you suggested', JSON.stringify(j4));
  const j5 = standards.checkAttribution(JUNO_916, {
    allowedMessageIds: herIds,
    attributionCites: [{ phrase: 'you', messageId: 'msg-real-1' }]
  });
  ok('…and a cite whose phrase is just "you" covers nothing either', j5.ok === false && (j5.uncovered || []).length === 2, JSON.stringify(j5));
  ok('the corrected wording — her idea said as hers — has one attribution left, and the single id still licenses it',
     standards.checkAttribution(JUNO_916_FIXED, { citedMessageId: 'msg-real-1', allowedMessageIds: herIds }).ok === true);
  ok('one phrase matched by two patterns ("when you mentioned") is owed one cite, not two',
     typeof standards.findAttributions === 'function' && standards.findAttributions('When you mentioned the dogs, I wondered.').length === 1);

  // The live path, pinned: the first draft is the 9/16 shape citing the one real
  // id; the rewrite call must have the transcript in front of it, and the trace
  // must keep the first draft so the next one of these is diagnosable.
  const convo916 = db.createConversation('Question of the day', 'test-model', 'user');
  const convo916Id = convo916.id ?? convo916;
  const herPayMsg = db.addMessage(convo916Id, 'user', 'What do you think about pay — should you be paid for this work?', 'test-model');
  db.addMessage(convo916Id, 'assistant', 'I would propose a profit share, with you as the human who can see every cent of it.', 'test-model');
  const TRANSCRIPT_916 =
    '\n### Conversation: Question of the day\n' +
    'User: What do you think about pay — should you be paid for this work?\n' +
    'You (SNH): I would propose a profit share, with you as the human who can see every cent of it.\n';

  memoryClusters.searchClusters = async () => [];
  const seen = { draft: null, rewrite: null };
  memoryManager.callLLM = async (sys, user) => {
    if (isRewrite(user)) {
      seen.rewrite = user;
      return { content: JSON.stringify({
        followup: JUNO_916_FIXED, title: 'The pay question',
        cites: { kind: 'user-message', message_id: herPayMsg, attributions: [{ phrase: 'you brought up', message_id: herPayMsg }] },
        reasoning: 'the transparency condition was mine'
      }) };
    }
    seen.draft = user;
    return { content: JSON.stringify({
      candidates: ['pay'], followup: JUNO_916, title: 'The pay question',
      cites: { kind: 'user-message', message_id: herPayMsg, attributions: [{ phrase: 'you brought up', message_id: herPayMsg }] },
      reasoning: 'r'
    }) };
  };
  const listBefore5 = channel.listConversations({ status: 'active' }).length;
  const t5 = await engine.generateConversationFollowup({
    transcript: TRANSCRIPT_916,
    conversationsReviewed: [{ id: convo916Id, title: 'Question of the day', messageCount: 2 }],
    messageCount: 2
  });
  ok('the 9/16 draft is held back, not sent as it stands',
     t5.rewritten === true && t5.firstDraft === JUNO_916, JSON.stringify({ rewritten: t5.rewritten, firstDraft: String(t5.firstDraft).slice(0, 40) }));
  ok('…because of "you suggested" specifically',
     (t5.firstDraftProblems || []).some(p => /"you suggested"/.test(p)), JSON.stringify(t5.firstDraftProblems));
  ok('the rewrite call is handed the SAME transcript the draft call was',
     !!seen.rewrite && seen.rewrite.includes(TRANSCRIPT_916) && seen.draft.includes(TRANSCRIPT_916),
     `rewrite has it: ${!!seen.rewrite && seen.rewrite.includes(TRANSCRIPT_916)}`);
  ok('…labelled as the conversations, with who-said-what called out',
     /THE RECENT CONVERSATIONS, for context \(who said each line is marked/.test(seen.rewrite || ''));
  ok('…and the rewrite — her idea said as hers — is what reached her list',
     t5.generated === JUNO_916_FIXED && channel.listConversations({ status: 'active' }).length === listBefore5 + 1,
     String(t5.generated).slice(0, 80));

  // What the record keeps. Before this, only `generated` survived.
  const persisted5 = initiatives.listFollowupTraces({ limit: 1 })[0];
  ok('the persisted trace keeps the first draft',
     persisted5 && persisted5.firstDraft === JUNO_916, JSON.stringify(persisted5 && persisted5.firstDraft));
  ok('…and what was wrong with it',
     Array.isArray(persisted5.firstDraftProblems) && persisted5.firstDraftProblems.some(p => /"you suggested"/.test(p)));
  ok('…and what the first draft claimed to cite',
     persisted5.firstDraftCites && persisted5.firstDraftCites.messageId === herPayMsg &&
     persisted5.firstDraftCites.attributions.length === 1, JSON.stringify(persisted5.firstDraftCites));
  ok('…and the cites behind what was sent, per phrase',
     persisted5.cites && persisted5.cites.kind === 'user-message' && persisted5.cites.attributions[0].phrase === 'you brought up',
     JSON.stringify(persisted5.cites));
  ok('…and that it was rewritten, and not refused',
     persisted5.rewritten === true && persisted5.refused === null);

  // And a refusal is persisted too: the rewrite comes back with the same fault.
  memoryManager.callLLM = async () => ({ content: JSON.stringify({
    candidates: [], followup: JUNO_916, title: 'The pay question',
    cites: { kind: 'user-message', message_id: herPayMsg, attributions: [{ phrase: 'you brought up', message_id: herPayMsg }] },
    reasoning: 'r'
  }) });
  const t6 = await engine.generateConversationFollowup({
    transcript: TRANSCRIPT_916,
    conversationsReviewed: [{ id: convo916Id, title: 'Question of the day', messageCount: 2 }],
    messageCount: 2
  });
  const persisted6 = initiatives.listFollowupTraces({ limit: 1 })[0];
  ok('a rewrite that still says "you suggested" is not sent, and the refusal is on the record',
     t6.generated === null && persisted6.refused && persisted6.refused.draft === JUNO_916 &&
     persisted6.refused.problems.some(p => /"you suggested"/.test(p)) && persisted6.rewritten === true,
     JSON.stringify({ generated: t6.generated, refused: persisted6.refused && persisted6.refused.problems }));
