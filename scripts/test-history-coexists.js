...
// ---------------------------------------------------------------------------
exempt('H1 pilot/cr-v — the trade vs what is owned now',
  'User kept the Pilot Elite for six months before trading it for a 2023 CR-V Hybrid.',
  'User owns a CR-V Sport');

exempt('H2 cr-v+ridgeline/cr-v — the one that took a death with it',
  'User had to get rid of the CR-V and the Ridgeline due to painful memories associated with a move.',
  'User has a CR-V');
...
//     exempted H2 against "User has a CR-V" — then the next corrector pass
//     retired it again for "User's CR-V is brand new", which is the same claim in
//     a different dress. The RAV/Ridgeline fact was retired twice by two spellings
//     of one mistake, which is why these cases exist.
// ---------------------------------------------------------------------------
exempt('H4 cr-v+ridgeline vs "User\'s CR-V is brand new" — the second retirement',
  'User had to get rid of the CR-V and the Ridgeline due to painful memories associated with a move.',
  "User's CR-V is brand new");

exempt('H5 truck trade vs "User\'s Frontier is brand new"',
  'User traded a truck in for a 2015 Frontier Limited.',
  "User's Frontier is brand new");

exempt('H6 cr-v trade vs "User\'s CR-V is brand new"',
  'User traded a CR-V in for a Pilot Elite.',
  "User's CR-V is brand new");

check('P1 "User\'s CR-V is brand new" reads as current state',
  rules.isCurrentState("User's CR-V is brand new"), true);
...
// participle. Caught firing live in a corrector pass: "User's CR-V has not had
// wax applied" consumed "CR-V has not" as the noun phrase, matched "had", and
// was filed as history — a statement about the car's condition right now.
check('P5 "User\'s CR-V has not had wax applied" is current state, not history',
  rules.isHistorical("User's CR-V has not had wax applied.").historical, false);

check('P6 …and reads as current state',
  rules.isCurrentState("User's CR-V has not had wax applied."), true);

check('P7 "User\'s Frontier had 200k miles" is still history',
  rules.isHistorical("User's Frontier had 200k miles").historical, true);

judged('N2 two present-perfect claims about the same thing still compete',
  "User's CR-V has not had wax applied.",
  "User's CR-V has had a ceramic coating applied.");
...
judged('C2 two past-tense claims still compete',
  'User owned a Frontier in 2019',
  'User had never owned a Frontier');
...
check('T5 past wins over present when both could match',
  rules.isCurrentState('User used to have a Frontier'), false);
...
judged('N1 past vs a non-state sentence is not exempted',
  'User traded the Miata for a 2026 Frontier',
  'Inn At Example Cove (IEC) is located in Example City on the Oregon coast');
