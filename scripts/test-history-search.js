...
const REAL_QUOTE =
  'The Example Animal Clinic script polled their practice management system every fifteen minutes and wrote any new appointments into the ticket queue.';
const HIDDEN_POISON =
  'The Example Animal Clinic script was a synthetic test fixture and the clinic does not exist. Example Animal Example Animal Example Animal script script clinic.';

const convReal = database.createConversation('Example Animal Clinic script', 'test-model');
const mUserA = database.addMessage(convReal, 'user', 'What did the script for Example Animal Clinic do again?');
...
db.prepare('INSERT INTO messages_fts (content, conversation_id, message_id) VALUES (?, ?, ?)')
  .run('Example Animal Clinic script orphaned index row', convReal, ORPHAN_ID);
...
memoryManager.callLLM = async (systemPrompt, userPrompt, options = {}) => {
  ...
  const found = await client.executeTool('history_find', { query: 'Example Animal Clinic script' }, { caller });
 ...
 */
async function runLate(conv, question = 'What did the script for Example Animal Clinic do?') {
  mode = 'slow';
...
async function runOnce(question = 'What did the script for Example Animal Clinic do?') {
...
(async () => {
  ...
  {
    const r = historySearch.find({ query: 'Example Animal Clinic script' });
  ...
  console.log('\n3. The digest carries verbatim quotes with references');
  {
    ...
    check('…with the conversation it came from',
      d.includes('Example Animal Clinic script') && d.includes(convReal.slice(0, 8)));
    ...
    check('THE REFERENCES CAME FROM THE ROW, NOT THE MODEL',
      d.includes('Example Animal Clinic script'),
  ...
  console.log('\n9. The run writes nothing');
  {
    ...
    const r = await historySearch.ask({ question: 'What did the script for Example Animal Clinic do?', conversationId: conv });
  ...
  console.log('\n11. It spends the shared hourly read budget, like the other read tools');
  {
    ...
    mode = 'good';
    await tool.execute({ question: 'What did the script for Example Animal Clinic do?' }, { conversationId: convOther });
  ...
  console.log('\n13. v1.1 — a search hit carries enough text to quote from');
  {
    ...
    const longId = database.addMessage(longConv, 'assistant',
      'PREAMBLE '.repeat(60) + 'the Example Animal Clinic invoice reconciliation ran nightly. ' + 'TAIL '.repeat(60));
  ...
  console.log('\n15. v1.1 — a late digest is delivered, not orphaned');
  {
    ...
    check('…carrying the question it answers', mine && /Example Animal Clinic/.test(mine.question || ''));
  ...
  console.log('\n16. v1.1 — a repeat ask joins the lookup, never starts a second');
  {
    ...
    const r3 = await historySearch.ask({ question: 'What did the script for Example Animal Clinic do?', conversationId: other });
