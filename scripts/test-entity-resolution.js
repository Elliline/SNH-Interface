...
(async () => {
  ...
  entities.resolveMentions('My dog Maple is a Beagle and she needs her shots.');
  const maple = entities.resolve('Maple');
  ok('an animal was created', maple.tier === 'clear' && maple.entity.type === 'animal',
     `${maple.tier}/${maple.entity && maple.entity.type}`);
  ...
  const r6b = entities.resolveMentions('Tidepool called to reschedule.');
  ...
  for (const [label, text] of [
    ...
    ['header block', 'From: jordan@example.com\nTo: Ellie\nSubject: phones\nRe: the open house']
  ]) {
  ...
  const rP = entities.resolveMentions('IEC is getting Opera from Oracle next month.');
  ...
  const hotelOrg = entities.create({ name: 'Inn at Example Cove', type: 'organization', aliases: ['IEC'] });
  entities.resolveMentions('Inn at Example Cove runs Opera for the front desk.');
  ...
  ok('the uses link exists', entities.usesOf(hotelOrg.id).some(e => e.id === opera.id),
     JSON.stringify(entities.usesOf(hotelOrg.id).map(e => e.name)));
  ok('…and reads from the other side too', entities.usersOf(opera.id).some(e => e.id === hotelOrg.id));
  ok('the uses link is ledgered',
     !!sql.prepare("SELECT 1 FROM corrections_ledger WHERE action='entity-link' AND target_id=? AND survivor_id=?")
       .get(hotelOrg.id, opera.id));

  // A SECOND client on the same product links to the same row.
  const clinic2 = entities.create({ name: 'Example Animal Clinic', type: 'organization', aliases: ['EAC'] });
  entities.resolveMentions('Example Animal Clinic runs Opera as well.');
  ...
  entities.resolveMentions('The clinic is on Vetline Cloud now.');
  const vetline = entities.resolve('Vetline Cloud').entity;
  ok('a product with no maker cue is still created', vetline && vetline.type === 'product',
     vetline && vetline.type);
  ok('…with NO maker invented for it', vetline && !vetline.org_id, vetline && vetline.org_id);
  ...
  const hotel = hotelOrg;
  const animalClinic = clinic2;
  ...
  ok('JUNO CASE 1 — the IEC street address still does NOT verify', vA.verified === false, vA.detail);

  const vB = subjectCheck.check("User's business email is eac2009@example.com",
