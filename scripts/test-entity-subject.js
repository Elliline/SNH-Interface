 ...
 * Example Animal Clinic email, neither catchable by any audit because
...
123 NE EXAMPLE HWY
Example City, OR 00000
Phone: (555) 010-0100
email: eac2009@example.com`;
...
(async () => {
  ...
  const hotel = entities.create({ name: 'Inn at Example Cove', type: 'org', aliases: ['IEC'], relationship: 'client' });
  const animalClinic = entities.create({
    name: 'Example Animal Clinic', type: 'org',
    aliases: ['EAC', 'the clinic'], relationship: 'client'
  });
  const known = [hotel, animalClinic, user, self].map(subjectCheck.forEntity);

  // 3a. The address fact. Subject was the user; the claim names IEC and carries
  // the clinic's street address. IEC appears NOWHERE in the source.
  const badAddress =
    'User works at IEC (Inn At Example Cove), a business located at 123 NE Example Hwy, Example City, OR 00000, ' +
    'and her role is effectively IT manager, involving leading teams and mediating between vendors like Acme POS and Example Payments.';
  const r3a = subjectCheck.check(badAddress, subjectCheck.forEntity(user), CLINIC_EMAIL_SOURCE, { knownEntities: known });
  ok('JUNO CASE 1 — the IEC street address does NOT verify', r3a.verified === false, JSON.stringify(r3a.detail));
  ok('JUNO CASE 1 — names the unattributable specific', /123|Example Cove|IEC/i.test(r3a.detail || ''), r3a.detail);

  // The same claim filed against IEC itself must also fail: the source never
  // mentions Inn at Example Cove at all.
  const r3aIsh = subjectCheck.check(
    'Inn at Example Cove is located at 123 NE Example Hwy, Example City, OR 00000',
    subjectCheck.forEntity(hotel), CLINIC_EMAIL_SOURCE, { knownEntities: known });
  ok('JUNO CASE 1b — the address does not verify against IEC either', r3aIsh.verified === false, r3aIsh.detail);
  ...
  const badEmail = "User's business email is eac2009@example.com";
  ...
  const goodEmail = subjectCheck.check(
    "Example Animal Clinic's email is eac2009@example.com",
    subjectCheck.forEntity(animalClinic), CLINIC_EMAIL_SOURCE, { knownEntities: known });
  ...
  const c1 = subjectCheck.check(
    'User works at IEC (Inn At Example Cove)', subjectCheck.forEntity(user),
    'I plan on going upstairs to get a sandwich. Im working at IEC (Inn At Example Cove) today and they have a restraunt named Tidepool.',
    { knownEntities: known });
  ok('"works at IEC" verifies from the message where she said it', c1.verified === true, c1.detail);

  const c2 = subjectCheck.check(
    'User is based in Example City, Oregon', subjectCheck.forEntity(user),
    'Yeah, we are based in Example City Oregon. and thank you for not saying im so sorry',
    { knownEntities: known });
  ok('"based in Example City" verifies from first person', c2.verified === true, c2.detail);
  ...
  const c5 = subjectCheck.check(
    "User's phone is (555) 010-0100", subjectCheck.forEntity(user), '', { knownEntities: known });
  ...
  const f1 = seed('Inn at Example Cove runs Shift4 for payments', 'user', 'hotel-cluster');
  sql.prepare('UPDATE cluster_members SET subject_entity_id = ? WHERE id = ?').run(hotel.id, f1);
  ok('getFactsForEntity finds it', entities.getFactsForEntity(hotel.id).some(f => f.id === f1));
  ...
  ok('still found when filed under the WRONG cluster (Juno\'s visibility failure)',
     entities.getFactsForEntity(hotel.id).some(f => f.id === f1));
  ...
  ok('exact alias is a clear match', entities.resolve('IEC').tier === 'clear');
  ok('containment is a clear match', entities.resolve('Inn at Example Cove').tier === 'clear');
  ...
  const dupe = entities.create({ name: 'Example Cove Inn', type: 'org', aliases: ['ECI'] });
  const dFact = seed('Example Cove Inn replaced its APs in August', 'user');
  ...
  const before = entities.getFactsForEntity(hotel.id).length;
  const merged = entities.merge(dupe.id, hotel.id, { reason: 'the same hotel under two names' });
  ok('the absorbed entity keeps no facts', entities.getFactsForEntity(dupe.id).length === 0);
  ok('every fact moved, none retired', entities.getFactsForEntity(hotel.id).length === before + 1);
  ok('the loser name survives as an alias', entities.namesOf(merged.survivor).includes('example cove inn'));
  ok('the loser alias survives too', entities.namesOf(merged.survivor).includes('eci'));
  ok('the lock carried over rather than being re-derived', entities.isEntityLocked(hotel.id, 'name'));
  ...
  ok('the merged entity is marked, not deleted',
     entities.get(dupe.id).status === 'merged' && entities.get(dupe.id).merged_into === hotel.id);

  // ------------------------------------------------------------- 8. identity
  section('8. Per-entity locks and the self pointer');

  let ptrErr = null;
  try { entities.setPointer('self', hotel.id, { actor: 'test' }); } catch (e) { ptrErr = e.message; }
  ...
  let repointErr = null;
  try { entities.repointFact(lockedMember, hotel.id); } catch (e) { repointErr = e.message; }
  ...
  entities.repointFact(ordinary, hotel.id, { reason: 'this was about the hotel, not about her' });
  ok('an ordinary fact repoints', sql.prepare('SELECT subject_entity_id FROM cluster_members WHERE id=?').get(ordinary).subject_entity_id === hotel.id);
