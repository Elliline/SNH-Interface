...
// AND A STUB BINARY, so nothing here reaches the real engine either. Until
// 2026-09-17 every dispatch in this suite spawned a REAL squatch-job on the
// box's real binary with the refactor brief below, eight of them per run,
// each holding a model stream on the live engine for as long as the brief
// took — and the suite never waited for or reaped them. The stub writes the
// report shape the dispatcher reads and exits at once; the one test that
// wants the failure path names a binary that does not exist.
const stubBin = path.join(projectsRoot, 'squatch-job-stub');
fs.writeFileSync(stubBin, [
  '#!/bin/sh',
  'out=""; prev=""',
  'for a in "$@"; do if [ "$prev" = "--report-json" ]; then out="$a"; fi; prev="$a"; done',
  'cat >/dev/null',
  '[ -n "$out" ] && printf \'%s\' \'{"status":"ok","report":"stub: nothing was changed.","facts":{"tool_calls":0}}\' > "$out"',
  'exit 0', ''
].join('\n'));
...
cfgMod.getConfig = () => {
  const c = origGet();
  c.tools = c.tools || {};
  c.tools.codingJobs = { ...(c.tools.codingJobs || {}), enabled: true, projectsRoot, binary };
  return c;
};
...
(async () => {
  ...
  await test('a missing squatch-job binary does not throw, and reports', async () => {
    binary = path.join(projectsRoot, 'no-such-squatch-job');
    let out;
    try {
      out = await codingJobs.runDispatched(
        { title: 'squatch-code: demo', task: 'x' },
        { timeoutMs: 5000 });
    } finally { binary = stubBin; }
