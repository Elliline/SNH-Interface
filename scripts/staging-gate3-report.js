...
(async () => {
  ...
  // corpus holding no fact about Biscuit. Gate 2 flagged it as a vacuous pass;
  // this checks it is not one any more.
  const f4 = (fixtures.results || []).find(r => r.id === 'F4');
  const biscuitCount = (() => {
    const n = (f4 && (f4.notes || []).find(x => /active facts naming Biscuit/.test(x))) || '';
    const m = String(n).match(/:\s*(\d+)\s*$/);
    return m ? parseInt(m[1], 10) : null;
  })();
  const f4Vacuous = !!(f4 && f4.pass && biscuitCount === 0);
  ...
  const report = {
    ...
    fixtures: { pass: fixtures.pass, f4Vacuous, biscuitCount },
  ...
  say('');
  if (f4Vacuous) {
    say('  F4 IS STILL VACUOUS — staging holds no fact naming Biscuit, so the rule it');
    say('  asserts (no subset sitting beside its superset) has nothing to be true about.');
  } else {
    say(`  F4 is no longer vacuous: staging holds ${biscuitCount} fact(s) naming Biscuit, so the`);
    say('  subset rule is being tested rather than satisfied by an empty set. That was');
    say("  gate 2's one weak pass, and carrying the Biscuit fact is what fixed it.");
  }
