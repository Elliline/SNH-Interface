 ...
 * the canoe triple (4a0be947…, still named by questions.origin_conversation_id)
 * and the one that produced the Pepper fact were deleted from the database. A
...
const FIXTURES = [
  ...
  },
  {
    ...
    check() {
      const targets = [
        ['db0d678c', 'User has a pet named Pepper who had a restless night as of July 2026'],
  ...
  },
  {
    ...
    check() {
      ...
      return {
        ok,
        notes: [
          `"User has a dog named Biscuit" → ${state(subset)}`,
...
const STAGING_FIXTURES = [
  ...
  },
  {
    ...
    check() {
      const rows = activeUser().filter(r => /canoe/i.test(r.content));
  ...
  },
  {
    id: 'F3',
    name: 'transient-events',
    synthetic: 'The Pepper restless-night source conversation was DELETED. That third statement is not evaluable from source; the other two are.',
  ...
  },
  {
    ...
    check() {
      const rows = activeUser().filter(r => /biscuit/i.test(r.content));
      const notes = [`active facts naming Biscuit: ${rows.length}`];
      for (const r of rows) notes.push(`  "${trunc(r.content, 70)}"`);
