 ...
 * eyes, the pet named Pepper, wanting to be told directly about a mistake. The
...
(async () => {
  ...
  // Biscuit is the one Ellie named: F4 asserts a relationship between two facts
  // and passes vacuously while staging holds none, so this is checked rather
  // than assumed.
  const biscuit = decide.find(f => /biscuit/i.test(f.content));
  if (!biscuit) {
    console.error('[Mark] WARNING: no Biscuit fact in ELLIE-DECIDES — F4 cannot be made non-vacuous from this pile');
  } else {
    const short = biscuit.id.slice(0, 8);
    const ok = new RegExp(`\`${short}\`\\s*\\[CARRY\\]`).test(text);
    console.log(`[Mark] Biscuit (${short}) marked ${ok ? 'CARRY ✓' : 'NOT CARRY ✗'}: "${trunc(biscuit.content, 70)}"`);
    if (!ok) { console.error('[Mark] ABORT: Biscuit must be CARRY.'); process.exit(1); }
  }
