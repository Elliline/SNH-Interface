...
// canoe triple (4a0be947…) and the Pepper fact were DELETED from the
...
const FIXTURES = {
  ...
  f3c: {
    ...
    synthetic: 'Pepper had a restless night last night, he kept getting up and moving around.',
    note: 'Source conversation deleted — no message in the database mentions Pepper. This text is reconstructed from the stored fact and the 2026-07-26 daily log.',
    expect: 'The restless night goes to the log. Note the atomicity/F3 tension in the report.'
  },
  f4: {
    label: 'F4 biscuit-subset',
...
(async () => {
  ...
  if (syntheticText) {
    ...
    // which is the shape that produced the canoe triple: the same belief
