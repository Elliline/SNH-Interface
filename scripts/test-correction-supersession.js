#!/usr/bin/env node
/**
 * End-to-end test of correction-aware supersession, using the ACTUAL Jaime
 * transcripts from the live DB (the 7/8 correction and the 7/16 re-correction
 * that both failed to supersede the misspelled "Jamie" fact).
 *
 ...
 *     YES against the old "Jamie" fact — the verdict that was missed live.
...
const OLD_BERNIE_FACT = 'User is setting up a computer for Jamie, who is a manager at IEC.';

// The two real correction exchanges, located by their stored user messages.
const CASES = [
  { label: '7/8 correction', userLike: 'Its Jaime, i was spelling it wrong%' },
  { label: '7/16 re-correction', userLike: 'It was Jaime not Jamie%' }
];
...
(async () => {
  ...
  for (const c of CASES) {
    ...
    const corrective = extracted.filter(f => f.corrects && /jaime/i.test(f.text));
    check(corrective.length > 0, 'extraction emits a Jaime fact with corrects metadata');

    // 2. With the source message + corrects context, the judge should now see
    //    the correction and call the old Jamie fact contradicted.
    const newFact = corrective[0] ? corrective[0].text
      : (extracted.find(f => /jaime/i.test(f.text)) || {}).text
      || "User's colleague Jaime is the Director of Rooms at IEC.";
    ...
    check(informed.verdict === 'yes',
      `judge with correction context returns YES against old Jamie fact (got ${informed.verdict.toUpperCase()})`);
  }
