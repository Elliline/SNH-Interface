 ...
 * described A containing B. "User owns a 2026 Frontier" was reported as containing
 * "User traded the Miata and the 2015 Frontier for a new 2026 Frontier", which it
...
async function classify() {
  ...
  for (const f of missing) {
    ...
    // judgeStripTheTimestamp, because "her grandfather died on 3 March 2019"
...
function writeReview({ missing, piles, seededAt, liveTotal, carriedThenChanged = [], carriedTotal = 0 }) {
  ...
  if (!piles.drop.length) {
    ...
    p('reasoning that described the opposite. It reported `User owns a 2026 Frontier` as');
    p('containing `User traded the Miata and the 2015 Frontier for a new 2026 Frontier`, which');
  ...
  if (archived.length) {
    ...
    p('> this group really are yours: the brown eyes, the pet named Pepper, being told');
