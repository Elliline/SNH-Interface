 ...
 * The findings-by-phase document a phased job wrote as it ran — the runner's
 * checkpoint, in her documents folder. Same rule as /file: BY JOB ID, NEVER BY
 * PATH; the location is looked up in the row and nothing a browser sends is
 * joined onto it. Served inline as text: it is markdown she reads, not a
 * download, and it exists while the job is still running — that is the point
 * of it — so a running job's card can open it too.
 */
router.get('/:id/findings', (req, res) => {
  try {
    ...
    const job = agentJobs.getJob(id);
    if (!job) return res.status(404).json({ error: 'No such job' });
    if (!job.findings_path) return res.status(404).json({ error: 'This job did not run in phases, so it has no findings document — its result is on the card.' });
    let stat;
    try { stat = fs.statSync(job.findings_path); } catch {
      return res.status(410).json({ error: 'The findings document is no longer where it was saved. The finished phases are still on the card.' });
    }
    if (!stat.isFile()) return res.status(410).json({ error: 'That is not a file any more.' });
    ...
    res.setHeader('X-Content-Type-Options', 'nosniff');
    res.setHeader('Cache-Control', 'private, no-store');
    fs.createReadStream(job.findings_path)
      .on('error', (err) => {
        console.error('[JobsAPI] Error streaming findings:', err.message);
        if (!res.headersSent) res.status(500).json({ error: 'Failed to read the document' });
        else res.destroy();
      })
      .pipe(res);
  } catch (error) {
    console.error('[JobsAPI] Error serving findings:', error.message);
    if (!res.headersSent) res.status(500).json({ error: 'Failed to serve the document' });
  }
});

/**
 * GET /api/jobs/window
 * What the job runner will plan against right now: the engine's context
 * window (or the pinned one), the configured reservation, and the room that
 * leaves. Read by Settings so the number that killed the 9/17 job is on a
 * screen. Nothing here is a setting; it is the derived truth.
 */
router.get('/window', async (req, res) => {
  try {
    ...
    const win = await require('../db/job-window').engineWindow();
    const answer = Math.max(64, gen.agentJobResponseTokens ?? 8192);
    const thinking = Number.isFinite(gen.agentJobThinkingTokens) ? gen.agentJobThinkingTokens : 0;
    const ctx = (c.agentJobs && c.agentJobs.context) || {};
    const floor = (Number.isFinite(ctx.floorAnswerTokens) ? ctx.floorAnswerTokens : 1024) + (Number.isFinite(ctx.floorThinkingTokens) ? ctx.floorThinkingTokens : 1024);
    res.json({
      window: win.window, source: win.source, model: win.model || (c.models && c.models.heartbeat && c.models.heartbeat.model) || null,
      answerTokens: answer, thinkingTokens: thinking, reservation: answer + thinking,
      promptRoomAtFullReservation: win.window ? Math.max(0, win.window - answer - thinking - (ctx.marginTokens ?? 512)) : null,
      promptRoomAtFloor: win.window ? Math.max(0, win.window - floor - (ctx.marginTokens ?? 512)) : null,
      floor
    });
  } catch (error) {
    console.error('[JobsAPI] Error reading the window:', error.message);
    res.status(500).json({ error: 'Failed to read the window' });
  }
});
